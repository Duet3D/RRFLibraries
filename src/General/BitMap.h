...
public:
	...
	void Iterate(std::function<void(unsigned int) /*noexcept*/ > func) const noexcept;
...
// Iterate over the bits
template<class BaseType> void Bitmap<BaseType>::Iterate(std::function<void(unsigned int) /*noexcept*/ > func) const noexcept
{
