...
public:
	...
	c_string c_str() const noexcept { return storage; }
	...
	int printf(c_string fmt, ...) noexcept __attribute__ ((format (printf, 2, 3)));
	int vprintf(c_string fmt, va_list vargs) noexcept;
	int catf(c_string fmt, ...) noexcept __attribute__ ((format (printf, 2, 3)));
	int vcatf(c_string fmt, va_list vargs) noexcept;
	bool copy(c_string src) noexcept { return GetRef().copy(src); }	// returns true if buffer is too small
	bool copy(c_string src, size_t maxlen) noexcept { return GetRef().copy(src, maxlen); }	// returns true if buffer is too small
	bool cat(c_string src) noexcept { return GetRef().cat(src); }		// returns true if buffer is too small
	bool catn(c_string src, size_t n) noexcept { return GetRef().catn(src, n); }		// returns true if buffer is too small
	bool cat(char c) noexcept { return GetRef().cat(c); }				// returns true if buffer is too small
	bool Prepend(c_string src) noexcept;								// returns true if buffer is too small

	void CopyAndPad(c_string src) noexcept;
	...
	bool Insert(size_t pos, c_string s) noexcept { return GetRef().Insert(pos, s); }	// returns true if buffer is too small
	bool Equals(c_string s) const noexcept { return strcmp(storage, s) == 0; }
	bool EqualsIgnoreCase(c_string s) const noexcept { return StringEqualsIgnoreCase(storage, s); }
	...
	bool Similar(c_string s) const noexcept { return strncmp(s, storage, Len) == 0; }
...
template<size_t Len> void String<Len>::CopyAndPad(c_string src) noexcept
{
...
template<size_t Len> inline int String<Len>::vprintf(c_string fmt, va_list vargs) noexcept
{
	return GetRef().vprintf(fmt, vargs);
}

template<size_t Len> inline int String<Len>::vcatf(c_string fmt, va_list vargs) noexcept
{
	return GetRef().vcatf(fmt, vargs);
}

template<size_t Len> inline bool String<Len>::Prepend(c_string src) noexcept
{
	return GetRef().Prepend(src);
}

template<size_t Len> int String<Len>::printf(c_string fmt, ...) noexcept
{
	va_list vargs;
	va_start(vargs, fmt);
...
template<size_t Len> int String<Len>::catf(c_string fmt, ...) noexcept
{
	va_list vargs;
	va_start(vargs, fmt);
...
template<size_t Len> int String<Len>::Contains(c_string s) const noexcept
{
	c_string_or_null const p = strstr(storage, s);
	return (p == nullptr) ? -1 : not_null(p) - storage;
}

template<size_t Len> int String<Len>::Contains(char c) const noexcept
{
	c_string_or_null const p = strchr(storage, (int)c);
	return (p == nullptr) ? -1 : not_null(p) - storage;
}
